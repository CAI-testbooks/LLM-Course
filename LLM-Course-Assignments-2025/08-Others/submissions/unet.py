...
class Pipe:
    ...
    def __init__(self, diffusion_prior=None, scheduler=None, device='cuda', modality=None,
                 train_superpixel_path='../../Data/processed_images/training_images_slic.pt',
                 train_spectrogram_path='../../Data/processed_images/training_images_sp.pt',
                 test_superpixel_path='../../Data/processed_images/test_images_slic.pt',
                 test_spectrogram_path='../../Data/processed_images/test_images_sp.pt'):
        ...
        self.device = device
        self.train_superpixel_dict = torch.load(train_superpixel_path) if train_superpixel_path else {}
        self.train_spectrogram_dict = torch.load(train_spectrogram_path) if train_spectrogram_path else {}
        self.test_superpixel_dict = torch.load(test_superpixel_path) if test_superpixel_path else {}
        self.test_spectrogram_dict = torch.load(test_spectrogram_path) if test_spectrogram_path else {}

    def _select_features(self, img_paths, img_features, text_features, mode='train'):
        """根据模态选择相应的特征"""
        if self.modality == 'image':
            return img_features
        elif self.modality == 'superpixel':
            # 根据模式选择训练或测试字典
            superpixel_dict = self.train_superpixel_dict if mode == 'train' else self.test_superpixel_dict
            # 从路径中提取文件名作为字典键
            img_names = [os.path.basename(path) for path in img_paths]
            return torch.stack([superpixel_dict[name] for name in img_names])
        elif self.modality == 'spectrogram':
            spectrogram_dict = self.train_spectrogram_dict if mode == 'train' else self.test_spectrogram_dict
            # 从路径中提取文件名作为字典键
            img_names = [os.path.basename(path) for path in img_paths]
            return torch.stack([spectrogram_dict[name] for name in img_names])
    ...
    def train(self, dataloader,
        ...
        for epoch in range(num_epochs):
            ...
            for batch in dataloader:
                (x, label, text, text_features, img, img_features) = batch
                x = x.to(device)

                ##### * 10 FOR keeping basic observable ####
                selected_features = (self._select_features(
                    img, img_features, text_features,mode='train'
                ) * 10.0).to(device)
                ##### * 10 FOR keeping basic observable ####
            ...
            with torch.no_grad():
                for batch in testdataloader:
                    ...
                    x = x.to(device)
                    selected_features = (self._select_features(
                        img, img_features, text_features, mode='test'
                    ) * 10.0).to(device)
                    c_embeds = eeg_model(x).type(torch.float32).to(device)
                    h_embeds = selected_features.type(torch.float32).to(device)
                    N = h_embeds.shape[0]
