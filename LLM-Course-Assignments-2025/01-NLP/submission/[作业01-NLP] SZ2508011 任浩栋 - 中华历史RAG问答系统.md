# 中国历史RAG问答系统

## 一.项目概述
作为南航数学学院的一位研究生，我对中国的历史颇感兴趣，因此，本项目旨在构建一个完全本地化、领域专精的中国历史智能问答系统，以 Qwen2.5-7B-Instruct 为基础模型，结合 RAG（Retrieval-Augmented Generation） 技术和 LoRA 监督微调，显著提升模型在中国历史领域的专业性、回答准确率和事实性。

### 1.1 项目核心目标
- **1.** 实现从数据采集 → QA 对生成 → LoRA 微调 → RAG 问答的全链路闭环
- **2.** 通过多次迭代优化 chunking 策略、微调参数、评估指标，解决大模型在历史领域常见的幻觉、事实性错误和回答泛化问题
- **3.** 全部运行于单张 A100 40GB 显卡，成本可控，适合个人/小团队复现

### 1.2 项目最终核心成果
- **1.** 一个支持流式输出的 Streamlit Web 问答界面
- **2.** 微调后模型在测试集上的 Precision、Recall、F1 指标显著提升，幻觉率得到有效控制

### 1.3 项目个人仓库地址(代码可复现)
- **github仓库**：https://github.com/renhaodong1011/chinese-history-rag-qwen 。

### 1.4 项目各次迭代所做的工作
- **迭代1**：1.编写了脚本model_download.py 用于下载Qwen-2.5-7B-Instruct模型到AutoDL本地；2.在文件夹data_extract中，编写脚本spider.py用于在中华上下五千年网站中爬取所有文本数据，共282个txt文 件；3.在文件夹data_extract中，编写脚本generate_QA_data.py,使用QWen4-8B模型，智能从爬取的所有数据中生成QA对，用于后续的Lora微调，共计提取QA对9978条，保存在相应文件夹下的chinese_history_qa.json文件中；4.在RAG.py文件中通过streamlit实现了一个完全本地化的中国历史领域 RAG（Retrieval-Augmented Generation）Web问答系统,支持流式输出。 且该第一版项目已经在AutoDL,A100-PCIE-40GB(40GB)单卡下部署成功。并将一些项目过程中的输出以截图的方式展示在了img文件夹中，包括数据浏览，QA对生成，RAG问答截图等。 后续的迭代，将以Lora微调模型为主线展开。
- **迭代2**： 1.新增Lora_tran.py 文件对Qwen-2.5-7B-Instruct模型进行Lora微调；2.新增merge_model.py 文件对微调后的模型进行合并； 3.将RAG中的模型替换为微调后的模型。 对搜集的9978条QA对进行微调在A100-PCIE-40GB(40GB)单卡模型下耗时2.5小时，一共训练了3个epoch, batch_size为4，结果显示Traning loss 和 Evaluation loss 曲线总体上呈现持续下降，Traning loss从一开始的2.9左右持续下降到0.5，Evaluation loss 从1.5左右持续下降到了0.3。最终在测试时，微调后RAG系统回答的更加精准以及简洁。
- **迭代3**: 新增对微调前后的Qwen模型进行评估，评估指标为Precision、F1和幻觉率。1.data_extract/generate_test_data.py: 使用Qwen-3-8B模型生成测试集数据保存在data_extract/chinese_history_test.json文件中 2. evaluation.py:对微调前面的模型进行评估。 3.eval/precision_F1_eval.py : 编写函数读取微调前后的模型，对测试集问题进行推理，并使用bret_score模块评估precision和F1。 eval/hallucination_eval.py：采用 LLM-as-a-Judge评估微调前和微调后的幻觉率，编写函数使用Qwen-3-8B模型，对微调前后模型生成的内容进行幻觉率判断。 实验结果表明微调后模型在测试集上的precision从原来的0.5946提升到了0.6721，F1分数从原来的0.6483提升到了0.7030。 但幻觉率没有显著提升。相关结果将由截图的形式展示在后面。
- **迭代4**: 新增测试集数据，从原先的100条测试集数据新增到280条，并修改幻觉率检测的prompt,之前的prompt认为模型回答的完全正确才会认为没有幻觉，修改后认为基本上正确没有事实性错误则没有幻觉。最终结果微调后模型在测试集上的precision从原来的0.6088提升到了0.6855，F1分数从原来的0.6561提升到了0.7080。 但幻觉率微调前为0.175，微调后为0.232。 目前怀疑有可能在微调时训练的轮数过多，导致模型存在过拟合的现象，后续将调整微调策略，看是否可以有效降低幻觉率，并进一步提高历史专业领域的precision和F1。
- **迭代5**: 本次迭代，使用了更强大的Qwen2.5-32B-Instruct模型，重新生成了QA对，并划分了训练集和测试集。生成的训练集QA对为14000条，测试集QA对150条。并在MAX_SEQ_LENGTH=1024，R=64，LORA_ALPHA=128的配置下重新微调了模型。完善了评估模块，对于precision,recall,f1等指标，舍弃了之前使用的bret_score方法，采用词级无序 F1 评估方法并加上LLM-judge方法进行修正。最终得到的结果为precision指标从0.526上升到了0.766， recall指标从0.669上升到了0.820， f1指标从0.566上升到了0.807，且幻觉率从微调前的0.2下降到0.107。
- **迭代6**：第六次迭代，重新考虑了chunking策略,采用RecursiveCharacterTextSplitter(chunk_size=800 chars, overlap=100)的策略，其优点为：递归按层级分隔符（默认 \n\n → \n → 空格 → 字符）切割，尽量保持段落/句子完整，避免随意断句。实现简单、速度快、计算开销低。overlap=100（约12.5%）能有效防止上下文跨 chunk 丢失。适合叙述性历史文本（如纪传体、编年体）。并考虑使用了分层/父子 Chunking策略，先切大parent chunk（完整章节/事件，1000-2000 token），再切小 child chunk（200-500 token）用于检索。检索时先找 child，再返回对应 parent 提供完整上下文。 并将RAG系统中的Qwen-2.5-7B模型换成了微调后的模型。

## 二.团队分工
- **任浩栋** ：负责：1.爬虫数据采集、2.QA对生成、3.Lora微调、4.模型评估、5.rag系统实现等模块的代码编写。
- **唐佳懿** ：负责：1.对数据进行审核和清洗， 2.Lora微调一次未达预期后对数据重新采集和生成QA对， 3. chunking策略探索与修改

## 三.运行环境

- **平台** ： AutoDL(https://www.autodl.com/)。
- **镜像配置**： PyTorch==2.8.0 Python==3.12(ubuntu22.04) CUDA==12.8。
- **GPU** ： A100-PCIE-40GB(40GB)。

## 四.依赖安装

**执行命令**: pip install -r requirement.txt


## 五.数据来源与处理
### 5.1 原始知识库数据（RAG 检索用）
-**来源** ：中华上下五千年网站（https://zhonghua.5000yan.com/）
-**采集方式** ：自定义爬虫（spider.py），共爬取 282 个 txt 文件，涵盖中国历史主要朝代、人物、事件。文本结构清晰，适合历史叙述类内容
-**运行spider.py截图**：<img width="913" height="474" alt="3" src="https://github.com/user-attachments/assets/47838b0b-06fe-4aa9-8b7a-f12079ab6562" />

### 5.2 监督微调数据（LoRA 训练用）
-**第一版** ：使用 Qwen-3-8B 从 282 个 txt 文件中智能生成 QA 对，共 9978 条，保存为 chinese_history_qa.json
-**第二版** ：升级至 Qwen2.5-32B-Instruct 重新生成，并独立划分，训练集 14000 条，测试集 150 条。
-**运行generate_QA_data.py获取QA对截图**：<img width="1920" height="1050" alt="10" src="https://github.com/user-attachments/assets/88dbac7b-f67c-427e-994d-5b2025ee6fa1" />

## 六.基础模型
使用 Qwen-2.5-7B-instruct 为基础模型
在AutoDL中使用model_download.py文件在 /root/autodl-tmp中下载完整的模型。
``` Python
import torch
from modelscope import snapshot_download, AutoModel, AutoTokenizer
import os
model_dir = snapshot_download('qwen/Qwen2.5-7B-Instruct', cache_dir='/root/autodl-tmp', revision='master')
```
- **模型目录**
<img width="245" height="412" alt="1" src="https://github.com/user-attachments/assets/cbc17021-6fa0-4fcd-9284-7a0b5c74d6b0" />

## 七.文件作用介绍
- **1.model_download.py**:用于下载Qwen-2.5-7B-Instruct模型到AutoDL本地。
- **2.data_extract/spider.py**: 用于在中华上下五千年网站中爬取所有文本数据，共282个txt文件。
- **3.data_extract/generate_QA_data.py**:使用QWen4-8B模型，智能从爬取的所有数据中生成QA对，用于后续的Lora微调，共计提取QA对9978条，保存在相应文件夹下的chinese_history_qa.json文件中。
- **4.RAG.py**: 通过streamlit实现了一个完全本地化的中国历史领域 RAG（Retrieval-Augmented Generation）Web问答系统,支持流式输出。
- **5.Lora_train.py**: 对Qwen-2.5-7B-Instruct模型进行Lora微调。
- **6.merge_model.py**: 对微调后的模型进行合并。
- **7.data_extract/generate_test_data.py**:使用Qwen-3-8B模型生成测试集数据保存在data_extract/chinese_history_test.json文件中。
- **8.evaluation.py**：对微调前面的模型进行评估。
- **9 eval/precision_F1_eval.py**：编写函数读取微调前后的模型，对测试集问题进行推理，采用词级无序 F1 评估方法并加上LLM-judge方法进行修正。
- **10 eval/hallucination_eval.py**：采用 LLM-as-a-Judge评估微调前和微调后的幻觉率，编写函数使用Qwen2.5-32B-Instruct模型，对微调前后模型生成的内容进行幻觉率判断。
- **11 inference.py**： 对微调前后的模型对测试集进行推理，并保存推理后的答案，便于进行模型评估。

## 八.启动方式
streamlit run RAG.py

## 九.方法
### 9.1 RAG 基础框架
-**文档加载**：TextLoader + utf-8 编码
-**文本切分策略（最终版）**：使用 RecursiveCharacterTextSplitter(chunk_size=800 chars, chunk_overlap=100)，优点：递归按自然段落/句子分割，语义完整性高，overlap 有效防止跨 chunk 信息丢失。迭代过程中也考虑过使用分层/父子 Chunking策略。
-**检索器** ：Chroma 向量数据库 + BAAI/bge-base-zh-v1.5 嵌入模型，k=6

### 9.2 LoRA 微调
-**基座模型** ：Qwen2.5-7B-Instruct（后续实验也尝试 Qwen2.5-32B）
-**训练参数（最终版）** ：r=64, lora_alpha=128, max_seq_length=1024, batch_size=4, epochs=3
-**训练时长**：单卡 A100 40GB，约 3.5 小时（14000条数据）

### 9.3 评估方法
-**Precision / Recall / F1** ：采用词级无序F1 + LLM-judge 修正(Qwen2.5-32B-Instruct)
-**幻觉率**：LLM-as-a-Judge（Qwen2.5-32B-Instruct 作为裁判），改进 prompt 后更宽松（允许合理推断，只要无事实性错误即判无幻觉）

## 十.实验结果
### 10.1 主要指标对比表
| 版本                          | Precision | Recall | F1     | 幻觉率 | 备注                     |
|-------------------------------|-----------|--------|--------|--------|--------------------------|
| 微调前（基座）                | 0.526     | 0.669  | 0.566  | 0.200  | 初始版本                 |
| 微调后（第一次微调）          | 0.6855    | -      | 0.7080 | 0.232  | prompt 宽松前            |
| 微调后（第二次微调）          | 0.766     | 0.820  | 0.807  | 0.107  | 最佳版本，显著提升       |


## 十一. demo截图 & 训练过程、评估截图等
### 11.1 运行截图（第一版，未微调）
<img width="1912" height="972" alt="6" src="https://github.com/user-attachments/assets/c5db1c26-2f3a-4c7c-a515-7e9d235b7a52" />
<img width="1912" height="972" alt="7" src="https://github.com/user-attachments/assets/9f3cdd56-7c10-4a87-9949-b31f412c98e2" />
<img width="1912" height="972" alt="8" src="https://github.com/user-attachments/assets/ffa8f600-aea7-48d3-ac39-5a2121e2f63e" />
<img width="1912" height="972" alt="9" src="https://github.com/user-attachments/assets/1322e128-cc37-4ec0-8f2a-fc86bdb6dfc5" />

### 11.2 第一次Lora 微调
- **训练过程、结果和可视化** ：
  <img width="1553" height="654" alt="12" src="https://github.com/user-attachments/assets/224f36e7-9cdc-489d-be7e-85110d3cbe59" />
  <img width="1222" height="504" alt="11" src="https://github.com/user-attachments/assets/d3bc4b58-ff10-4860-80e3-554431a6d5a4" />
  <img width="552" height="391" alt="13" src="https://github.com/user-attachments/assets/ceb9b62f-e5e3-46a7-bfe9-20c6480db1f4" />

### 11.3 第一次微调前后模型评估
- **测试集数据** ：
  <img width="1503" height="542" alt="16" src="https://github.com/user-attachments/assets/9a9039b2-eeb5-43a0-9f2e-128ab15b326a" />
- **评估过程** ：
  <img width="1541" height="402" alt="19" src="https://github.com/user-attachments/assets/e09b173c-d690-40a8-8b0d-6d1dfd00bb24" />
- **评估结果可视化**
  <img width="531" height="415" alt="20" src="https://github.com/user-attachments/assets/bb20fbcb-21a2-432e-b557-009097a4652d" />

### 11.4 第二次优化后的Lora微调和模型评估

<img width="1200" height="490" alt="22" src="https://github.com/user-attachments/assets/32081f4a-846b-4282-91b1-6dfd48e1c5af" />
<img width="2964" height="1763" alt="24" src="https://github.com/user-attachments/assets/57e6279e-db49-4ce1-aab5-f558414b68ea" />
- **训练微调后RAG运行截图**：
  <img width="1920" height="1050" alt="14" src="https://github.com/user-attachments/assets/39a801a6-db85-43e6-967d-f79adea4f746" />
  <img width="1920" height="1050" alt="15" src="https://github.com/user-attachments/assets/e0b3e539-9bad-4065-b03f-385c352d9e4f" />


## 十二. 问题分析与创新点
-**主要问题及解决**：
-**1**.早期版本：回答泛化、事实性错误多 → 通过 LoRA + 大量高质量 QA 对针对性微调显著改善。
-**2**.幻觉率一度上升 → 改进 LLM-judge prompt（允许合理推断），幻觉率从 0.232 降至 0.107。
-**3**.上下文过长导致模型崩溃 → 采用 chunk_size=800 + overlap=100 + k=6 策略，有效控制输入长度。
-**4**.评估不准 → 舍弃 bert-score，采用词级无序 F1 + LLM-judge 组合，更贴近历史领域真实质量。

-**创新点**：
-**1**.全链路本地化实现：从爬虫 → QA 生成 → LoRA 微调 → RAG 部署，一条龙在单卡 A100 上完成。
-**2**.多轮迭代优化：从 9978 条 → 14000 条训练数据，从 7B → 尝试 32B 生成数据，指标持续提升。
-**3**.评估体系创新：结合词级 F1 + LLM-judge，更加客观地衡量历史专业性与事实性。

## 十三. 未来改进方向
-**继续降低幻觉率** ：
尝试使用DPO / ORPO 等对齐算法，进一步对齐事实性。可以考虑引入 RAG + self-reflection 机制，模型自我检查回答。
-**提升检索质量**：
实验 parent-child chunking + HyDE（假设文档增强）以及混合检索（向量 + BM25 关键词）。
-**更大规模数据与模型**：
尝试 Qwen2.5-32B / 72B 基座进行全参数或更大 r 的 LoRA并且扩展数据源：加入二十四史原文、地方志、考古资料。
-**更全面评估**：
引入人工评估 + 多模型交叉验证，并增加多轮对话能力测试。
