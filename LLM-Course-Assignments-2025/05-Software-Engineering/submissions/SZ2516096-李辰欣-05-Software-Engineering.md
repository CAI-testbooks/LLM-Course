...
为了便于快速运行 DevAgent 并复现实验，我们为其开发了一个命令行工具（CLI，Command Line Interface）。该工具基于 Python argparse 模块实现，可在命令行中输入任务描述或任务文件，自动完成“需求理解 → 代码生成 → 测试执行 → 缺陷修复”循环。

CLI 提供 solve 子命令，实现任务求解功能，支持以下参数：

| 参数            | 说明                     |
| ------------- | ---------------------- |
| `-t / --task` | 直接输入任务描述               |
| `-f / --file` | 从文件读取任务描述              |
| `--max-iter`  | 设置 Agent 最大迭代次数，覆盖默认配置 |
| `--save`      | 将最终生成代码保存到指定文件         |
| `--verbose`   | 打印每轮迭代详细信息             |

使用截图

<img width="1047" height="539" alt="Snipaste_2026-01-10_20-26-23" src="https://github.com/user-attachments/assets/3b5eda00-742c-4d73-aa46-dbd8b581c229" />


** 设计优势 **

- 可复现性：CLI 与 DevAgent 紧密结合，每次运行任务可得到确定的迭代日志与最终代码。

- 可扩展性：支持多任务输入，可与 HumanEval、MBPP 等基准数据集结合做批量实验。

- 易用性：无需改动 Agent 逻辑，即可通过命令行完成任务求解。
