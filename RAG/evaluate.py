...
class EvalResult:
    ...
    # 查询重写信息
    rewritten_queries: List[str] = field(default_factory=list)  # 重写后的查询
    rewrite_mode: str = None      # 使用的重写模式
...
def run_comparison(args):
    ...
    测试配置组合：
    1. baseline: 无重写、无重排序
    2. rewrite_single: 单查询重写
    3. rewrite_multi: 多查询重写
    4. rewrite_hyde: HyDE 假设文档嵌入
    5. rerank_only: 仅重排序
    6. rewrite_single_rerank: 单查询重写 + 重排序
    7. rewrite_hyde_rerank: HyDE + 重排序
    """
    configs = [
        {"name": "baseline", "enable_rewrite": False, "rewrite_mode": "single", "enable_rerank": False},
        {"name": "rewrite_single", "enable_rewrite": True, "rewrite_mode": "single", "enable_rerank": False},
        {"name": "rewrite_multi", "enable_rewrite": True, "rewrite_mode": "multi", "enable_rerank": False},
        {"name": "rewrite_hyde", "enable_rewrite": True, "rewrite_mode": "hyde", "enable_rerank": False},
        {"name": "rerank_only", "enable_rewrite": False, "rewrite_mode": "single", "enable_rerank": True},
        {"name": "rewrite_single_rerank", "enable_rewrite": True, "rewrite_mode": "single", "enable_rerank": True},
        {"name": "rewrite_hyde_rerank", "enable_rewrite": True, "rewrite_mode": "hyde", "enable_rerank": True},
    ]
    ...
    for cfg in configs:
        ...
        evaluator = RAGEvaluator(
            ...
            use_llm_judge=False,  # 比较模式不使用LLM评判（太慢）
            seed=args.seed
        )
    ...
    print("=" * 70)
    print(f"{'Config':<25} {'Hit@1':>8} {'Hit@5':>8} {'MRR':>8} {'Time(s)':>10}")
    print("-" * 70)
    
    for r in all_results:
        print(f"{r['config']:<25} {r['hit_at_1']:>8.4f} {r['hit_at_5']:>8.4f} {r['mrr']:>8.4f} {r['elapsed']:>10.2f}")
    
    print("-" * 70)
    
    # 找出最佳配置
    best_mrr = max(all_results, key=lambda x: x["mrr"])
    best_hit1 = max(all_results, key=lambda x: x["hit_at_1"])
    
    print(f"\n🏆 Best MRR: {best_mrr['config']} ({best_mrr['mrr']:.4f})")
    print(f"🏆 Best Hit@1: {best_hit1['config']} ({best_hit1['hit_at_1']:.4f})")
...
def main():
    ...
    parser.add_argument("--rewrite-mode", default="single",
                        choices=["single", "multi", "context", "auto", "hyde"],
                        help="查询重写模式")
    parser.add_argument("--enable-cache", action="store_true", help="启用查询缓存")
    
    # 重排序配置
    parser.add_argument("--enable-rerank", action="store_true", help="启用重排序")
    parser.add_argument("--rerank-model", default="BAAI/bge-reranker-base", help="重排序模型")
    parser.add_argument("--rerank-top-k", type=int, default=None, help="重排序后返回数量")
    
    # 比较模式
    parser.add_argument("--compare", action="store_true", 
                        help="比较多种配置的效果（自动测试多种组合）")
    ...
    if args.enable_rewrite:
        print(f"  Query Rewrite: {args.rewrite_mode}")
    if args.enable_rerank:
        print(f"  Rerank: {args.rerank_model}")
